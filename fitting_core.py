...
def fit_package_help():
    print(
        ...
        " in x_fit\n"
        "fp.plot(x_fit, fit_func(x_fit, *params), new_fig=False) # it is "
        "important x_fit is a numpy array.\n"
        "# This next part plots the stderr of your fit as a shaded region.\n"
        "stderr = np.sqrt(np.diag(cov))\n"
        "plt.fill_between(x_fit, fit_func(x_fit, *(params - stderr)), "
        "fit_func(x_fit, *(params + stderr)), alpha=0.3, color='k')\n"
        "\n"
